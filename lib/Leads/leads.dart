...
class _LeadsState extends State<Leads> {
                ...
                builder: (context, snapshot) {
                  if (!snapshot.hasData) {
                    return SizedBox(
                      height: 100,
                      child: Center(
                          child: CircularProgressIndicator(color: Color(0xFF9EA700))),
                    );
                  }
                                    ...
                                    Container(
                                      ...
                                      child: DropdownButton<String>(
                                        ...
                                        onChanged: (String? newValue) {
                                          setDialogState(() {
                                            tempSelectedCreationDate = newValue == "None" ? null : newValue;
                                            if (newValue != "None") {
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      child: DropdownButton<String>(
                                        ...
                                        onChanged: (String? newValue) {
                                          setDialogState(() {
                                            tempSelectedClosedDate = newValue == "None" ? null : newValue;
                                            if (newValue != "None") {
  ...
  Future<void> leadData() async {
    try {
      List<dynamic> domain = [['type', '=', 'lead']];
      final filters = getFilters();

      // Build the domain based on selected filters
      for (String filterKey in selectedFilters) {
        ...
        } else if (filters.containsKey(filterKey)) {
          domain.addAll(filters[filterKey]!['domain'] as List<dynamic>);
        }
      }

      print("Constructed domain: $domain"); // Debug log to check domain
  ...
  Future<String> fetchTagNames(List<dynamic>? tagIds) async {
    ...
    try {
      ...
      if (response != null && response is List && response.isNotEmpty) {
        final List<Map<String, dynamic>> tags = List<Map<String, dynamic>>.from(response);
        final tagNames = tags.map((tag) => tag['name']?.toString() ?? 'Unknown').join(', ');
  ...
  Widget kanban(){
    return leadsList.isEmpty
      ...
      itemBuilder: (context, index) {
        ...
        final tagIds = lead['tag_ids'] is List ? lead['tag_ids'] as List<dynamic> : null;
                        ...
                        children: List.generate(
                          3, // Total number of stars
                            ...
                            color: starIndex < priorityLevel
                                ? Colors.amber // Filled star for priority level
  ...
  Future<void> fetchLeadActivities() async {
    try {
      ...
      for (var lead in leadsList) {
        ...
        if (lead['user_id'] != null && lead['user_id'] is List && lead['user_id'].length > 0) {
      ...
      final response = await client?.callKw({
        ...
        'kwargs': {
          'fields': ['activity_type_id', 'summary', 'date_deadline', 'state','user_id'],
        }
      });
      ...
      if (response != null) {
        List<Map<String, dynamic>> activities = List<Map<String, dynamic>>.from(response);
        for (var activity in activities) {
          if (activity['user_id'] != null && activity['user_id'] is List && activity['user_id'].length > 0) {
            ...
            if (!activityUserImages.containsKey(userId)) {
              ...
              activity['user_image'] = userImage != null ? base64Encode(userImage) : null;
            } else {
        ...
        for (var lead in leadsList) {
          if (lead['activity_ids'] != null && lead['activity_ids'] is List) {
            leadActivities[lead['id']] = activities.where((activity) => lead['activity_ids'].contains(activity['id'])).toList();
            print("Lead ${lead['id']} activities: ${leadActivities[lead['id']]}");
          }
        }
  ...
  Widget activityView() {
    ...
    List<Map<String, dynamic>> leadsWithActivities = leadsList
        .where((lead) => leadActivities.containsKey(lead['id']) && leadActivities[lead['id']]!.isNotEmpty)
        .toList();
    ...
    return SfDataGrid(
      rowHeight: 60,
      source: LeadActivityDataSource(leadsWithActivities, leadActivities, salespersonImages, activityTypes, context),
      columnWidthMode: ColumnWidthMode.fill,
...
class LeadActivityDataSource extends DataGridSource {
  ...
  LeadActivityDataSource(this.leads, this.leadActivities, this.salespersonImages, this.activityTypes, this.context);
  ...
  List<DataGridRow> get rows => leads.map((lead) {
    List<DataGridCell> cells = [
      DataGridCell<Map<String, dynamic>>(
        ...
        value: {
          ...
          'customer': lead['partner_id'] != null && lead['partner_id'] is List && lead['partner_id'].length > 1
              ? lead['partner_id'][1]
              : 'None',
          'salesperson_id': lead['user_id'] != null && lead['user_id'] is List && lead['user_id'].length > 0
              ? lead['user_id'][0]
              : null,
          'stage': lead['stage_id'] != null && lead['stage_id'] is List && lead['stage_id'].length > 1
              ? lead['stage_id'][1]
              : 'Unknown',
          'expected_revenue': lead['expected_revenue']?.toString() ?? '0.00',
          'id':lead['id'],
        },
      ),
    ];
    ...
    for (var type in activityTypes) {
      ...
      var matchingActivity = activitiesForLead.firstWhere(
            (activity) => activity['activity_type_id'] is List &&
      ...
      cells.add(
        DataGridCell<Map<String, dynamic>>(
          ...
          value: matchingActivity.isNotEmpty
              ? {
            'date_deadline': matchingActivity['date_deadline']?.toString() ?? '',
  ...
  Color _getActivityColor(String date) {
    ...
    try {
      ...
      DateTime activityDateStart = DateTime(activityDate.year, activityDate.month, activityDate.day);
    ...
    return DataGridRowAdapter(
      cells: row.getCells().map((cell) {
        if (cell.columnName == 'lead') {
          ...
          Uint8List? salespersonImage = salespersonId != null ? salespersonImages[salespersonId] : null;
