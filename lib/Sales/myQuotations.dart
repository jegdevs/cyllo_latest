...
class _MyquotationsState extends State<Myquotations> {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final creationDate = lead['create_date'] != null
            ...
            DateTime.parse(lead['create_date']))
            : '';
        final partnerId = lead['partner_id'];
        final customerName = (partnerId is List && partnerId.length > 1)
            ? partnerId[1]
            : 'None';
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: Container(
                          padding: EdgeInsets.all(12),
                          decoration: BoxDecoration(
                            color: hasActivity ? Colors.blue.shade50 : Colors.grey.shade100,
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(
                              color: hasActivity ? Colors.blue.shade200 : Colors.grey.shade300,
                            ),
                          ),
                          child: Row(
                            children: [
                              Container(
                                ...
                                height: 32,
                                decoration: BoxDecoration(
                                  color: hasActivity ? Colors.blue.shade100 : Colors.grey.shade200,
                                  borderRadius: BorderRadius.circular(8),
                                ),
                                child: Icon(
                                  getActivityIcon(activityType),
                                  size: 18,
                                  color: hasActivity ? Colors.blue.shade600 : Colors.grey.shade600,
                                ),
                              ),
                              SizedBox(width: 12),
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      'Activity',
                                      style: TextStyle(
                                        color: hasActivity ? Colors.blue.shade700 : Colors.grey.shade600,
                                        fontSize: 12,
                                        fontWeight: FontWeight.w500,
                                      ),
                                    ),
                                    ...
                                    Text(
                                      hasActivity
                                          ? (activitySummary!='false' ? activitySummary : activityType.toUpperCase())
                                          : 'No Activity',
                                      style: TextStyle(
                                        color: hasActivity ? Colors.blue.shade800 : Colors.grey.shade700,
                                        fontWeight: FontWeight.w600,
                                        fontSize: 14,
                                      ),
                                    ),
  ...
  Widget fetchData() {
    ...
    for (var lead in leadsList) {
      ...
      double revenue = double.tryParse(lead['amount_total']?.toString() ?? "0") ?? 0.0;
      ...
      double currentTotal = double.tryParse(groupedData[monthYear]!['total']) ?? 0.0;
      groupedData[monthYear]!['total'] = (currentTotal + revenue).toStringAsFixed(2);
    ...
    if (selectedChart == "bar") {
      return showNoDataMessage
        ...
        primaryXAxis: CategoryAxis(
          ...
          maximumLabels: 100, labelPlacement: LabelPlacement.onTicks,
          interval:  1.2,
        ),
  ...
  List<ChartData> prepareChartData(List<Map<String, dynamic>> salesOrders) {
    ...
    for (var order in salesOrders) {
      ...
      if (order['partner_id'] != false && order['partner_id'] is List && order['partner_id'].length > 1) {
